...
M.treesitter.config = tie("Plugin nvim-treesitter -> config", function(_, opts)
  ...
  tied.each_i(
    ...
    function(_, parser)
      if
        not vim.list_contains(installed, parser)
        and not vim.list_contains(extra_opts.ignore, parser)
      then
    ...
    callback = function(ev)
      ...
      local should_enable = tie(
        ...
        function(query)
          ...
          local lang_supports_query = (
            vim.treesitter.query.get(lang, query) ~= nil
          )
